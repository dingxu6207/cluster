...
CG18radec = np.loadtxt('Trumpler 5.txt')
CG18radec = CG18radec[CG18radec[:,7]>0.5]

from scipy.spatial import cKDTree
kdt = cKDTree(CG18radec[:,0:2])
dist, indices = kdt.query(VSXradec)

temp = []
for i in range (len(indices)):
    index = indices[i] 
    temp.append(CG18radec[index])
...
lista = ['INDEX', 'distance','VSXRA', 'VSXDEC', 'GRA', 'GDEC', 'PLX', 'PMRA', 'PMDEC', 'G', 'BP-RP', 'P']
...
#DOWNradec = allradec[np.argsort(allradec[:,1])]

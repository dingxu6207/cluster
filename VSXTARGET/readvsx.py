...
#GAIA的星表数据
data = np.loadtxt('Berkeley 18.txt')
print(len(data))
data = data[data[:,2]>0]
#data = data[data[:,2]<1]
...
kdt = cKDTree(datapro[:,0:2])

dist, indices = kdt.query(radec)

temp = []
for i in range (len(indices)):
    index = indices[i] 
    temp.append(datapro[index])
    
nptemp = np.array(temp)    

allradec = np.column_stack((radec, nptemp))

indexdata = np.arange(0,radec.shape[0])

allradec = np.column_stack((indexdata, allradec))

highradec = allradec[allradec[:,11]==0]
highradec[:,0] = highradec[:,0]+2
...
plt.scatter(highdata[:,6]-highdata[:,7], highdata[:,5], marker='o', color='green',s=10)
plt.scatter(highradec[:,9]-highradec[:,10], highradec[:,8], marker='o', color='red',s=5)
#plt.scatter(highradec[2,9]-highradec[2,10], highradec[2,8], marker='o', color='red',s=5)
